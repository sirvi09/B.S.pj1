...
const AddShows = () => { 
   ...
   const handleDateTimeAdd = () => {
    ...
    const [date, time] = dateTimeInput.split('T'); // "2025-07-15T14:30"
    if (!date || !time) return;

    setDateTimeSelection(prev => {
      const times = prev[date] ?? [];
      if (times.includes(time)) return prev;               // already added
    ...
    return nowPlayingMovies.length > 0 ? (
       ...
       <div className="overflow-x-auto pb-4">
        ...
        {/* FIXED DATETIME PICKER SECTION */}
        <div className="mt-6">
          <label className="mb-2 block text-sm font-medium">Select Date and Time</label>

          {/* 1. datetime-local picker - NOW VISIBLE */}
          <div className="inline-flex items-center gap-3 rounded-md border border-gray-600 p-2 bg-black">
            <input
              type="datetime-local"
              value={dateTimeInput}
              onChange={e => setDateTimeInput(e.target.value)}
              className="h-10 w-64 px-3 text-sm text-white-900 bg-black border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-primary focus:border-primary"
